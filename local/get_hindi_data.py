...
parser = argparse.ArgumentParser(description="IITM Hindi data pre-process")
parser.add_argument("--data_root", required=True, default=None, type=str)
parser.add_argument("--data_sets", default="dev", type=str)
...
def __process_transcript(file_path: str):
    """
    For a given wav file and transcript, capturing the metadata.
    Args:
        file_path: path to a source transcript  with wav sources
    Returns:
    ...
    with open(file_path, encoding="utf-8") as fin:
        for line in fin:
            ...
            wav_file = os.path.join(os.path.join(root, "audio_new"), id + ".wav")
            # check duration
...
def __process_data(data_folder: str, manifest_file: str, num_workers: int):
    ...
    with open(manifest_file, "w", encoding='utf8') as fout:
        for m in entries:
...
def main():
    ...
    for data_set in data_sets.split(","):
        ...
        filepath = os.path.join(data_root, data_set)
